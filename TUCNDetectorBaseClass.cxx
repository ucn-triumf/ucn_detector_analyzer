...
TUCNDetectorBaseClass::TUCNDetectorBaseClass(bool isOffline, bool isLi6, bool saveTree){
    ...
    // setup fHitsInCycleCumulIntime TH1D
    if(fIsLi6){
    ...
    // setup fHitsPerCycleHIntime TH1D
    if(fIsLi6){
...
void TUCNDetectorBaseClass::ProcessMidasEvent(TDataContainer& dataContainer){
    ...
    // Check for sequence start timeHitsTree
    ...
    if(sequence_started && fLastCycleStartTime != 0.0){
        ...
        fHitsPerCycleVectorIntime.push_back(std::pair<double, double>(fLastCycleStartTime,
                                                                      fTotalHitsCycleIntime));
        ...
        if(fIsLi6){
            std::cout << std::dec << "Li6 cycle finished at time "
            << now->tm_hour << ":" << now->tm_min << ":" << now->tm_sec
            << ";  total UCN = " << fTotalHitsCycle
            << ";  UCN in valve open time = " << fTotalHitsCycleIntime
            << std::endl;
        }else{
            std::cout << std::dec << "He3 cycle finished at time "
            << now->tm_hour << ":" << now->tm_min << ":" << now->tm_sec
            << ";  total UCN = " << fTotalHitsCycle
            << ";  UCN in valve open time = " << fTotalHitsCycleIntime
            << std::endl;
    ...
    for(unsigned int i = 0; i < fHitsPerCycleVector.size(); i++){
        ...
        if(fHitsPerCycleVector[i].second > maxCycle)
            maxCycle = fHitsPerCycleVector[i].second;
    }
