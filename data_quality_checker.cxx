...
public:
  ...
  void EndRun(int transition,int run,int time){
    ...
    if(time_synch_pulses.size() == 10){
      ...
      if(!IsOffline()) cm_msg(MERROR,"Data Quality","EOR check failed; there were %i V1725 time synch pulses (should be 10).\n",
    ...
    if(time_synch_pulses_chrono.size() == 10){
      ...
      if(!IsOffline()) cm_msg(MERROR,"Data Quality","EOR check failed; there were %i Chrono time synch pulses (should be 10).\n",
			    time_synch_pulses.size());
    ...
    if(good_number && good_number_chrono ){
      if(!IsOffline()) cm_msg(MINFO,"Data Quality","EOR check successful; %s",message.c_str());
    ...
    if(time_synch_pulses.size() && start_time != 0){
      ...
      if(!IsOffline()) cm_msg(MINFO,"Data Quality",
	...
	if(baselines_n[i] != 0){
	  ...
	  if((diff > 45 || diff < -25) && i < 9){
	    if(!IsOffline()) cm_msg(MINFO,"Data Quality","baseline check failed for channel %i; expected baseline 14718, actual baseline %f; new recommended offset %i.\n",
  ...
  bool ProcessMidasEvent(TDataContainer& dataContainer){
	...
	if((word & 0xff000000) == 0xff000000){ // find rollover word
	  ...
	  if(ch == 30 and !falling) time_synch_pulses_chrono.push_back(full_time);
	}
      }
    }
	...
	if(start_time_chrono == 0  && time_synch_pulses_chrono.size()) start_time_chrono = time_synch_pulses_chrono[0];
	...
	if(time_synch_pulses.size() == 10){
	  ...
	  message.append("#V1725 Time synch pulses = 10;");
          ...
          if(!IsOffline()) cm_msg(MERROR,"Data Quality","BOR check failed; there were %i V7125 time synch pulses (should be 10).\n",
		 time_synch_pulses.size());
	...
	if(time_synch_pulses_chrono.size() == 10){
	  ...
	  message.append("#Chrono Time synch pulses = 10;");
          ...
          if(!IsOffline()) cm_msg(MERROR,"Data Quality","BOR check failed; there were %i Chrono time synch pulses (should be 10).\n",
	...
	if(good_number && good_number_chrono){
	  if(!IsOffline()) cm_msg(MINFO,"Data Quality","BOR check successful; %s",message.c_str());
